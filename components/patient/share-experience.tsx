...
export default function ShareExperience({ fullName }: ShareExperienceProps) {
  ...
  const visibleStories = showOnlyQuestions ? stories.filter((entry) => entry.feeling.toLowerCase().includes('question')) : stories
        ...
        <div className="space-y-4">
          <Card className="rounded-[1.8rem] border-0 bg-[linear-gradient(180deg,#8b5cf6_0%,#c14fc8_100%)] text-white shadow-[0_24px_60px_rgba(177,79,200,0.28)]">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                <Button onClick={copyShareLink} className="w-full rounded-full bg-white text-[#0e5fd8] transition-all hover:bg-white hover:scale-[1.01] sm:w-auto">{copied ? <Check className="mr-2 h-4 w-4" /> : <Copy className="mr-2 h-4 w-4" />}{copied ? 'Copied' : 'Copy link'}</Button>
                <Button onClick={useNativeShare} variant="ghost" className="w-full rounded-full border border-white/25 bg-white/10 text-white transition-all hover:bg-white/15 hover:scale-[1.01] sm:w-auto"><Share2 className="mr-2 h-4 w-4" />Share now</Button>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-[1.8rem] border-0 bg-white shadow-[0_20px_50px_rgba(55,95,180,0.12)]">
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={publishStory} className="h-12 w-full rounded-full bg-[linear-gradient(90deg,#60a5fa_0%,#4f8ff8_100%)] shadow-[0_18px_35px_rgba(79,143,248,0.35)] transition-all hover:scale-[1.01]">Create Post →</Button>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="rounded-[1.8rem] border-0 bg-white shadow-[0_20px_50px_rgba(55,95,180,0.12)]">
          <CardHeader>
            <div className="flex items-center justify-between gap-3">
              ...
              <Button variant="ghost" size="icon" className="rounded-full border border-slate-200 transition-all hover:scale-[1.03]" onClick={() => setShowOnlyQuestions((value) => !value)}><Filter className="h-4 w-4" /></Button>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            {visibleStories.map((entry) => (
              <div key={entry.id} className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5">
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <Button variant="ghost" className="h-9 rounded-full px-4 text-[#0e5fd8] transition-all hover:bg-sky-50 hover:scale-[1.02]" onClick={() => router.push('/patient/dashboard')}>Read more</Button>
                </div>
              </div>
            ))}
          </CardContent>
