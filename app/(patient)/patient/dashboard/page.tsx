...
const quickReliefActions = [
  { label: 'Take a breath', icon: Wind, tab: 'tools' },
  { label: 'Shift your thinking', icon: Brain, tab: 'tools' },
  { label: 'Ground yourself', icon: Sparkles, tab: 'home' },
  { label: 'Take a small step', icon: Target, tab: 'goals' },
  { label: 'Get help', icon: HeartHandshake, tab: 'community' },
]
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="relative overflow-hidden rounded-[2.2rem] bg-white shadow-inner">
              <div className="bg-[linear-gradient(180deg,#9a59ff_0%,#7c74ff_100%)] px-5 pb-6 pt-5 text-white">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="icon" className="rounded-full bg-white/10 text-white transition-all hover:bg-white/15 hover:scale-[1.03]" onClick={() => setReliefOpen((open) => !open)}>
                      ...
                      {quickReliefActions.map((action) => {
                        ...
                        return (
                          <button
                            key={action.label}
                            type="button"
                            onClick={() => { setActiveTab(action.tab); setReliefOpen(false) }}
                            className="flex flex-col items-center gap-2 text-center transition-transform hover:scale-[1.03]"
                          >
              ...
              <TabsList className="absolute inset-x-4 bottom-4 z-40 grid h-20 grid-cols-5 rounded-[1.9rem] border border-slate-200 bg-white/95 p-2 shadow-[0_18px_50px_rgba(88,126,255,0.2)] backdrop-blur">
                ...
                <button
                  ...
                  className="-mt-8 flex h-16 w-16 items-center justify-center self-center justify-self-center rounded-full bg-[linear-gradient(180deg,#67b0ff_0%,#4f8ff8_100%)] text-white shadow-[0_18px_40px_rgba(79,143,248,0.45)] transition-transform hover:scale-[1.04]"
                >
